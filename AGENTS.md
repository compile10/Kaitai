...
│   ├── providers.ts         # PROVIDER_MAP and PROVIDERS array (all 5 AI provider configs)
│   └── types.ts             # WordNode, SentenceAnalysis, Provider, ModelInfo, ProviderConfig
│
...
│   └── contexts/
│       └── SettingsContext.tsx      # React Context for provider/model (localStorage)
│
...
| State | React Context + localStorage | Zustand + Expo SecureStore | — |
| Routing | Next.js App Router | Expo Router (file-based) | — |
| Visualization | @xyflow/react (React Flow) | Horizontal scroll cards | — |
| AI | LangChain (Anthropic, OpenAI, Google, xAI, OpenRouter) | — | Provider config |
...
|--------|---------|
| `main` | Primary branch |
| `react-native` | Active development branch (mobile app + multi-client architecture) |
| `multiple-llm-provider-support` | Feature branch for multi-provider support |

The current working branch is `react-native`.
...
- **No SSR for visualization**: `SentenceVisualization.tsx` and `SettingsModal.tsx` are client components (`"use client"`). React Flow does not support server rendering.
